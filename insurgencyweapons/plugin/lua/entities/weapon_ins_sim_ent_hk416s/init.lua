...
function ENT:SpawnFunction(ply, tr)
	...
	local ent = ents.Create("weapon_ins_sim_ent_hk416s")
		ent:SetPos(SpawnPos)
	ent:Spawn()
	ent:Activate()
...
function ENT:Initialize()
	...
	self.Entity:SetModel("models/weapons/j_rif_ha.mdl")
...
function ENT:Use(ply)
	if ply:IsPlayer() and not ply:HasWeapon("weapon_ins_sim_hk416s") then
		self.Entity:EmitSound("BaseCombatCharacter.AmmoPickup")
		ply:Give("weapon_ins_sim_hk416s")
		self:Remove()
	end
end
