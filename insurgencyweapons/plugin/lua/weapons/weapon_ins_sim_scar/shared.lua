...
SWEP.ViewModel			= Model("models/weapons/i_rif_scmed.mdl")
SWEP.WorldModel			= Model("models/weapons/j_rif_scmed.mdl")
SWEP.ViewModelFOV      = 65
SWEP.Spawnable			= false
SWEP.AdminSpawnable		= false

SWEP.Primary.Sound 		= Sound("Weapoi_SCAR.Single")
...
SWEP.IronSightsPos = Vector (-2.7112, -3.0003, 1.0492)
SWEP.IronSightsAng = Vector (0.165, 0.0208, 0)
...
function SWEP:Reload()
	...
	if (self.Owner:KeyDown(IN_USE)) then
		...
		if (SERVER) then
			...
			local knife = ents.Create("weapon_ins_sim_ent_scar")
...
//			end
			...
			self.Owner:StripWeapon("weapon_ins_sim_scar")
			if (SERVER) then
