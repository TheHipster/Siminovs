...
SWEP.PrintName			= "H&K HK416 W/ ACOG"					// 'Nice' Weapon name (Shown on HUD)	
...
if (file.Exists("materials/weapons/weapon_ins_sim_hk416s.vmt","GAME")) then
	SWEP.WepSelectIcon	= surface.GetTextureID("weapons/weapon_ins_sim_hk416s")
end
	...
	if (self.Sniper) then
		...
		if (bScope) then 
			...
			if (self.RedDot) then
				...
				surface.SetTexture(surface.GetTextureID("scope/crosshair_acog"))
			end
