...
SWEP.Category			= "Siminov's Base INSMINI"
...
SWEP.data.JamL		= "Weapon Magazine Jam"
...
function SWEP:ShootEffects()
	...
	timer.Simple(0, function()
		...
		if (self.Shotgun) then
			...
			util.Effect("effect_sim_gunsmoke_ins", effectdata)
		end
	end)
	end
...
function SWEP:CanPrimaryAttack()
	...
	if ( self.Weapon:Clip1() <= 0 ) and self.Primary.ClipSize > -1 or self:GetNWBool("BigJammed") or self:GetNWBool("LittleJammed") then
		if self.Weapon:GetDTBool(0) then return end
		self.Reloadaftershoot = CurTime() + self.Primary.Delay
		self.ActionDelay = (CurTime() + self.Primary.Delay + 0.05)
		self.Weapon:SetNextPrimaryFire(CurTime() + 1)
		if self:GetNWBool("BigJammed") or self:GetNWBool("LittleJammed") then
